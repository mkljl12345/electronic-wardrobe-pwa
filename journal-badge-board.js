(() => {
  ...
  const JEJU_APRIL23_LAYOUT = [
    { x: 18, y: 19, scale: 0.72, rotation: 0 },
    { x: 50, y: 18, scale: 0.74, rotation: 0 },
    { x: 82, y: 20, scale: 0.72, rotation: 0 },
    { x: 27, y: 48, scale: 0.78, rotation: 0 },
    { x: 72, y: 47, scale: 0.78, rotation: 0 },
  ...
  const JEJU_APRIL26_BADGES = [
    ...
    "./jeju-2026-04-26-badge-03.png",
    "./jeju-2026-04-26-badge-04.png",
  ];
  const JEJU_APRIL26_LAYOUT = [
    { x: 28, y: 28, scale: 0.9, rotation: -2 },
    { x: 72, y: 28, scale: 0.9, rotation: 2 },
    { x: 28, y: 70, scale: 0.9, rotation: 2 },
    { x: 72, y: 70, scale: 0.9, rotation: -2 },
  ];
  const urls = new Set();
  const cleanup = () => {
  ...
  const load = async (journalId, date) =>
  ...
  const seedJejuApril23 = async (record, journalTitle, date) => {
    ...
    const existing = new Set(record.badges.map((badge) => badge.id));
    const missing = JEJU_APRIL23_BADGES.map((src, index) => ({
      src,
      index,
      id: `jeju-april23-${index + 1}`,
    })).filter((item) => !existing.has(item.id));
    if (!missing.length) return record;
    const blobs = await Promise.all(
      missing.map(async (item) => {
    ...
    missing.forEach((item, position) =>
      record.badges.push({
        id: item.id,
        image: blobs[position],
        ...JEJU_APRIL23_LAYOUT[item.index],
        z: ++z,
      }),
    );
    await save(record);
  ...
  const seedJejuApril26 = async (record, journalTitle, date) => {
    ...
    const existing = new Set(record.badges.map((badge) => badge.id));
    const missing = JEJU_APRIL26_BADGES.map((src, index) => ({
      src,
      index,
      id: `jeju-april26-${index + 1}`,
    })).filter((item) => !existing.has(item.id));
    if (!missing.length) return record;
    const blobs = await Promise.all(
      missing.map(async (item) => {
    ...
    missing.forEach((item, position) =>
      record.badges.push({
        id: item.id,
        image: blobs[position],
        ...JEJU_APRIL26_LAYOUT[item.index],
        z: ++z,
      }),
    );
    await save(record);
    ...
    const context = canvas.getContext("2d", { willReadFrequently: true }),
      ...
      background = corners.reduce(
        (sum, [x, y]) => {
          ...
          return [
            sum[0] + data[offset] / 4,
            sum[1] + data[offset + 1] / 4,
            sum[2] + data[offset + 2] / 4,
          ];
        },
        [0, 0, 0],
      ),
    ...
    const threshold = 62,
      enqueue = (index) => {
        ...
        const offset = index * 4,
          dr = data[offset] - background[0],
          dg = data[offset + 1] - background[1],
          db = data[offset + 2] - background[2];
  ...
  const openCrop = (file, onDone) => {
    ...
    reader.onerror = () => alert("无法读取图片");
    reader.onload = () => {
      const source = new Image();
      source.onerror = () => alert("无法读取图片");
      source.onload = () => {
        const overlay = document.createElement("div");
        overlay.className = "journal-badge-crop-overlay";
        overlay.innerHTML =
          '<section class="journal-badge-crop"><header><strong>裁切单枚徽章</strong><button type="button">×</button></header><p>每张图片只保留一枚徽章。如果是合集图，请重复上传并逐个裁切。</p><div class="journal-badge-crop-frame"><img alt="徽章裁切"></div><small>单指拖动 · 双指捏合缩放</small><button type="button" class="journal-badge-crop-save">抠图并添加</button></section>';
        const frame = overlay.querySelector(".journal-badge-crop-frame"),
          preview = frame.querySelector("img");
        ...
        const update = () => {
          const size = frame.clientWidth,
            base = Math.max(size / source.width, size / source.height);
          preview.style.width = `${source.width * base}px`;
          preview.style.height = `${source.height * base}px`;
          preview.style.transform = `translate(calc(-50% + ${x}px),calc(-50% + ${y}px)) scale(${zoom})`;
        };
        const distance = (t) =>
            Math.hypot(
          ...
          begin = (t) => {
            gesture =
              t.length > 1
                ...
                : t.length
                  ? { kind: "drag", px: t[0].clientX, py: t[0].clientY, x, y }
                  : null;
          };
        frame.addEventListener("touchstart", (event) => begin(event.touches), {
          passive: true,
        });
        frame.addEventListener(
          ...
          (event) => {
            ...
            if (gesture.kind === "drag" && event.touches.length === 1) {
              x = Math.max(
                -180,
                Math.min(
                  180,
                  gesture.x + event.touches[0].clientX - gesture.px,
                ),
              );
              y = Math.max(
                -180,
                Math.min(
                  180,
                  gesture.y + event.touches[0].clientY - gesture.py,
                ),
              );
            } else if (gesture.kind === "pinch" && event.touches.length > 1) {
              ...
              x = Math.max(
                -180,
                Math.min(180, gesture.x + point.x - gesture.center.x),
              );
              y = Math.max(
                -180,
                Math.min(180, gesture.y + point.y - gesture.center.y),
              );
            } else begin(event.touches);
            update();
          },
          { passive: false },
        );
        frame.addEventListener("touchend", (event) => begin(event.touches), {
          passive: true,
        });
        ...
        overlay.querySelector(".journal-badge-crop-save").onclick =
          async () => {
            const size = 512,
              ...
              scale = (base * zoom * size) / frameSize,
            ...
            context.drawImage(
              source,
              (size - drawWidth) / 2 + (x / frameSize) * size,
              (size - drawHeight) / 2 + (y / frameSize) * size,
              drawWidth,
              drawHeight,
            );
            const result = trimToSquare(removeBackground(canvas));
            if (!result) {
              alert("未识别到徽章主体，请重新裁切。");
              return;
            }
      ...
      record.badges
        ...
        .forEach((badge) => {
          ...
          let timer = null,
            ...
            active = false;
          const promote = () => {
            badge.z = Math.max(0, ...record.badges.map((value) => value.z)) + 1;
            item.style.zIndex = String(badge.z);
          ...
          item.addEventListener(
            ...
            (event) => {
              ...
              } else if (start.kind === "pinch" && event.touches.length > 1) {
                const currentDistance = Math.hypot(
                  ...
                  angle =
                    (Math.atan2(
                      ...
                      180) /
                    Math.PI;
                badge.scale = Math.max(
                  0.35,
                  Math.min(3, (start.scale * currentDistance) / start.distance),
                );
                badge.rotation = start.rotation + angle - start.angle;
              } else {
                begin(event.touches);
                return;
              }
